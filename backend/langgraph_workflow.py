...
Optimized: Router uses keyword detection + default route (no LLM call).
...
def router_node(state: WorkflowState) -> WorkflowState:
    ...
    Optimized: uses keyword detection + default route (no LLM call ~90% of the time).
    """
    t0 = time.time()
    ...
    if meta_result:
        log_timing("router", time.time() - t0, "meta question")
        return {
            **state,
            "route": "meta",
    ...
    # Keyword detection + default route (no LLM)
    result = classify_query(state["query"], state["session_id"])
    route = result["route"]

    log_timing("router", time.time() - t0, f"route: {route} ({result.get('reason', '')})")
